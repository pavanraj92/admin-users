...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ isset($user) ? 'Edit ' . $role->name : 'Create ' . $role->name }}
    </li>
@endsection
        ...
        <div class="row">
            <div class="col-12">
                <div class="card card-body">
                    <form
                        action="{{ isset($user) ? route('admin.users.update', ['type' => $type, 'user' => $user->id]) : route('admin.users.store', ['type' => $type]) }}"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="status" class="form-control select2" required>
                                        @foreach (config('user.constants.status', []) as $key => $label)
                                            <option value="{{ $key }}"
                                                {{ (isset($user) && (string) $user?->status === (string) $key) || old('status') === (string) $key ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
